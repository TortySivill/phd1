...
def interact(f,X1,X2,x,means,cov,mini,maxi,e,X):
	...
	for p in range(0,1):
		...
		original_prediction = f(np.asarray(temp).reshape(1,-1))
		...
		new_prediction = f(np.asarray(temp).reshape(1,-1))

		sigma_1 = original_prediction - new_prediction


		original_prediction = f(np.asarray(means).reshape(1,-1))
...
def RDG(f,x,means,cov,mini,maxi,e,X):
	...
	f_expected = f(np.asarray(means).reshape(1,-1))
	...
	if len(seps) > 0:
		for var in seps:
				...
				atts[tuple(var)] = f(np.asarray(temp).reshape(1,-1)) - np.mean(f(X))
	...
	for var in nonseps:
		...
		if len(S) > 0 and len(barS) > 0:
			...
			if len(barS) > 0:
				atts[str(var)] = f(np.asarray(means).reshape(1,-1))
			elif len(S) > 0:
				atts[tuple(var)] = f(np.asarray(x).reshape(1,-1))[0] - np.mean(f(X))
